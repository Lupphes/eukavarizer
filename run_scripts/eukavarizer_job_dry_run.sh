#!/bin/bash
#========================================================================================
# PBS Job: Eukavarizer Pipeline - Homo sapiens (Human, Taxonomy 9606) - Dry Run
#========================================================================================
#PBS -N eukavarizer_human_9606_dryrun
#PBS -l select=1:ncpus=64:mem=512gb:scratch_local=400gb
#PBS -l walltime=24:00:00
#PBS -m abe
#PBS -M ondrej.sloup@protonmail.com
#PBS -j oe
#PBS -o /storage/brno2/home/luppo/logs/eukavarizer_human_9606_dryrun.log
...
# Profile: mix_medium (Delly, Tiddit, Sniffles, CuteSV)
# Run type: Dry run / validation

DATADIR=/storage/brno2/home/luppo
SCRATCH=$SCRATCHDIR
LOGFILE="$DATADIR/human_9606_dryrun/logs/eukavarizer.log"
...
echo "=== Eukavarizer Pipeline: Human (9606) - Dry Run ===" | tee -a "$LOGFILE"
...
export NXF_WORK="$DATADIR/human_9606_dryrun/work"
export NXF_LOG_FILE="$DATADIR/human_9606_dryrun/logs/.nextflow.log"
...
sed "s|\$DATADIR|$DATADIR|g" "$DATADIR/eukavarizer/assets/samplesheets/samplesheet_human_dry_run.csv" > "$SCRATCH/samplesheet_formatted.csv"

#========================================================================================
# Pipeline Execution
#========================================================================================
echo ">>> Running Eukavarizer pipeline..." | tee -a "$LOGFILE"
"$SCRATCH/nextflow" run main.nf -profile mamba,mix_medium,qc_off \
    --taxonomy_id 9606 \
    ...
    --outdir "$DATADIR/human_9606_dryrun/out" \
    --seqtk_size 1.0 --seqtk_flag false --bwamem2 false --minimap2_flag true | tee -a "$LOGFILE"
