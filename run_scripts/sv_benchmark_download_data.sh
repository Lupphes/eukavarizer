...
FTP_BASE="https://ftp-trace.ncbi.nlm.nih.gov/ReferenceSamples/giab/data/AshkenazimTrio/HG002_NA24385_son/PacBio_CCS_15kb/GRCh38_no_alt_analysis"
echo "Downloading BAM file (64GB)..." | tee -a "$LOGFILE"
wget -c "${FTP_BASE}/HG002.15kb.Sequel.pbmm2.GRCh38.whatshap.haplotag.RTG.10x.trio.bam" | tee -a "$LOGFILE"

echo "Downloading BAM index..." | tee -a "$LOGFILE"
wget -c "${FTP_BASE}/HG002.15kb.Sequel.pbmm2.GRCh38.whatshap.haplotag.RTG.10x.trio.bam.bai" | tee -a "$LOGFILE"

echo "=== Completed at $(date) ===" | tee -a "$LOGFILE"
echo "Data location: $DATADIR/SV_Benchmark_CMRG_GIAB" | tee -a "$LOGFILE"
echo "BAM file: $DATADIR/SV_Benchmark_CMRG_GIAB/giab_data/HG002.15kb.Sequel.pbmm2.GRCh38.whatshap.haplotag.RTG.10x.trio.bam" | tee -a "$LOGFILE"